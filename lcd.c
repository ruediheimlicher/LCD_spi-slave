...
*/
void lcd_initialize_spi(uint8_t set_function, uint8_t set_entry_mode, uint8_t on)
{
   /* 20ms delay while LCD powers on */
   _delay_ms(30);
   ...
   /* Set function and wait 40uS */
   //lcd_load_byte(set_function);
   lcd_send_spi_cmd(set_function);
   
   /* Turn off the display and wait 40uS */
   //lcd_load_byte(LCD_CMD_OFF);
   lcd_send_spi_cmd(LCD_CMD_OFF);
   
   /* Clear display and wait 1.64mS */
   //lcd_load_byte(LCD_CMD_CLEAR);
   lcd_send_spi_cmd(LCD_CMD_CLEAR);
   _delay_ms(4);
   
   /* Set entry mode and wait 40uS */
   //lcd_load_byte(set_entry_mode);
   lcd_send_spi_cmd(set_entry_mode);
   _delay_ms(4);
   /* Turn display back on and wait 40uS */
   //lcd_load_byte(on);
   lcd_send_spi_cmd(on);
...
void
lcd_send_spi_cmd(uint8_t out_cmd)
{
   
   /* Data  is a command, so RS must be low (0) */
...
lcd_inc_cursor(uint8_t spaces)
{
        while (spaces--)
        {
                ...
                lcd_send_spi_cmd(LCD_CMD_CURSOR_RIGHT);
        }
}
...
lcd_dec_cursor(uint8_t spaces)
{
        while (spaces--)
        {
                ...
                lcd_send_spi_cmd(LCD_CMD_CURSOR_LEFT);
        }
}
